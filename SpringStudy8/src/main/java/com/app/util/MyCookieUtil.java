...
public class MyCookieUtil {
	 ...
	 * cookieName) {
	 * 
	 * Cookie[] cookies = request.getCookies(); String value = null;
	 * 
	 * for (Cookie ck : cookies) { if (ck.getName().equals(cookieName)) { // value =
	 * ck.getValue(); //그냥 읽기 // value decode 처리해서 읽기
	 * 
	 * try { value = URLDecoder.decode(ck.getValue(), "UTF-8"); } catch
