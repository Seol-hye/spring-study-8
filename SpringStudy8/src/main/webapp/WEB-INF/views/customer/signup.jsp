...
<body>
	...
	<!-- 	모든 값을 입력받는 경우 -->
		<form action="" method="post">
			사용자아이디 : <input type="text" name="id" id="inputId">
	...
	<script>
		...
		btn_checkDupIdJson.addEventListener("click", ()=>{
			...
			$.ajax({
				...
				dataType: "json",	//서버에서 응답으로 보내는 포맷도 json으로 작업
				//dataType: "text",	//서버에서 응답으로 보내는 포맷도 json으로 작업
				success: function(result){
					console.log('ajax success');
					console.log(result); //	"Y"		"N"
					
					//dataType: text -> text로 인식(json format 되어있는)
					//dataType: json -> 응답이 json 인지 -> (javascript object 변환) -> result 주입
					
					//dataType: text result 들어있는 건? json format 으로 맞게 작성되어있는 텍스트(문자열)
					//텍스트 -> json 변환(파싱)
					//let jsObj = JSON.parse(result);
					
					//dataType: json result 들어있는 건? 이미 텍스트를 json 파싱해서 javascript object 변환해서 주입
